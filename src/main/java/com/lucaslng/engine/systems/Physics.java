...
public class Physics {
	...
	public static final float G = 0.000000980f;
	...
	public void step(double dt) {
		...
		for (int ida : entityManager.getEntitiesWith(PositionComponent.class, RigidBodyComponent.class, AABBComponent.class)) {
			...
			for (int idb : entityManager.getEntitiesWith(PositionComponent.class, RigidBodyComponent.class, AABBComponent.class)) {
		...
		for (int i = 0; i < contactIds.size(); i++) {
			...
			RigidBodyComponent ra = entityManager.getComponent(ida, RigidBodyComponent.class);
			Vector3f pa = entityManager.getComponent(ida, PositionComponent.class).position();
			Vector3f va = ra.isStatic() ? new Vector3f() : entityManager.getComponent(ida, VelocityComponent.class).velocity();
			RigidBodyComponent rb = entityManager.getComponent(idb, RigidBodyComponent.class);
			Vector3f pb = entityManager.getComponent(idb, PositionComponent.class).position();
			Vector3f vb = rb.isStatic() ? new Vector3f() : entityManager.getComponent(idb, VelocityComponent.class).velocity();
	...
	private static void resolveContact(RigidBodyComponent ra, RigidBodyComponent rb, Vector3f pa, Vector3f pb, Vector3f va, Vector3f vb, Vector3f c) {
		Vector3f rv = new Vector3f();
		va.sub(vb, rv); // relative velocity
		float rvAlongNormal = rv.x() * c.x() + rv.y() * c.y() + rv.z() * c.z();
		if (rvAlongNormal > 0) return; // moving apart
		...
		Vector3f t = new Vector3f(rv.x() - rvAlongNormal * c.x(), rv.y() - rvAlongNormal * c.y(), rv.z() - rvAlongNormal * c.z()); // tangent vector
		float tLen = t.length();
		if (tLen > 0.0001f) {
			t.div(tLen);
			float jt = -(rv.x() * t.x() + rv.y() * t.y() + rv.z() * t.z());
			jt /= invMassSum;
			float mu = (float) Math.sqrt(ra.friction() * ra.friction() + rb.friction() * rb.friction()); // combine friction
